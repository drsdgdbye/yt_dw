...
- Режимы скрипта: без флагов — видео; `--list <url>` — элементы поста; `--item N <url>` — конкретный элемент. Для `instagram.com/.../p|tv/<shortcode>/` Handler зовёт `List`: один элемент — сразу, несколько — инлайн-кнопки `1…N` и последняя `All` (callback data `ig:<shortcode>:<N>` и `ig:<shortcode>:all`), выбор обрабатывает `Handler.PickMedia`, `All` — `Handler.sendAllItems`. Тип медиа в кнопках не показывается. Reels и остальные ссылки идут обычным путём.
